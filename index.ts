...
export default function (pi: ExtensionAPI) {
	...
	pi.registerCommand("share-as", {
		...
		handler: async (args, ctx) => {
			...
			if (!sessionFile) {
				if (ctx.hasUI) ctx.ui.notify("Cannot share an in-memory session. Save or start a normal session first.", "error");
				return;
			}
			...
			try {
				...
				if (authResult.code !== 0) {
					if (ctx.hasUI) ctx.ui.notify("GitHub CLI is not logged in. Run 'gh auth login' first.", "error");
					return;
				}
			} catch {
				if (ctx.hasUI) ctx.ui.notify("GitHub CLI (gh) is not installed. Install it from https://cli.github.com/", "error");
				return;
			}
			...
			try {
				const result = await pi.exec(
					"gh",
					["gist", "create", "--public", "false", "--desc", requestedName, tmpFile],
					{ signal: ctx.signal },
				);

				if (result.code !== 0) {
					if (ctx.hasUI) ctx.ui.notify(`Failed to create gist: ${result.stderr.trim() || "Unknown error"}`, "error");
					return;
				}
